...
void AddObject(struct NodeTree* node, TYPE_TREE dif, TYPE_TREE correct, TYPE_TREE wrong) {
	node->data = dif;
...
void Comparison(struct NodeTree* node) {
	...
	for (size_t i = 0; i < list_1.size; i++) {
		if (!strcmp("NO", list_1.nodes[pos_1].value))
			printf(OUTPUT_FORMAT_TREE " ", list_1.nodes[pos_1].value);
		else
			printf(OUTPUT_FORMAT_TREE "\n", list_1.nodes[pos_1].value);
...
void PrintDef(struct List* list, int pos) {
	if (!strcmp("NO", list->nodes[pos].value))
		printf(OUTPUT_FORMAT_TREE " ", list->nodes[pos].value);
	else
